...
function LoadPrinterConfigurations(filePath)
    ...
    if not (printerConfig["sThePrinterLocation"] and printerConfig["sThePrinterLocationCT4"]
        ...
        error("Faltan configuraciones críticas: sThePrinterLocation, sThePrinterLocationCT4, sTester o sLine no están definidas en el archivo de configuración.")
    end
end
...
function DetermineShift(currentTime)
    ...
    if (hour > 6 or (hour == 6 and minute >= 30)) and (hour < 18 or (hour == 18 and minute < 30)) then
        return "D" -- Turno de día
    else
        return "N" -- Turno de noche
    end
end
--local currentTime = os.date("%H:%M") -- Obtener la hora actual
...
function PrintStringRAW15char()
    ...
    .."^FT1080,720,^BY5^BCI,260,N,N,N ^FD#07#^FS\n"           -- Número de parte cliente barcode
    .."^FT990,990^A0I,100,100^FH\\^FD#07#^FS\n"               -- Número de parte cliente leyenda
    .."^PQ1,0,1,Y^XZ\n"
    return sPrintStringRAW
end

-- Función para impresión de 14 caracteres
...
-- Función para convertir número de parte Ford
...
function AppendUnlockLog(pin, personName)
    local logPath = "\\\\mlxgumvwfile01\\Departamentos\\Fakra\\Pruebas\\CyclesCounter\\GeneralCounter\\UnlockedLog\\unlock_log.txt"
    local f = io.open(logPath, "a")
    if f then
        local timeStamp = os.date("%Y-%m-%d %H:%M:%S")
        f:write("🔓 Desbloqueo por ["..personName.."] (PIN: "..pin..") a las "..timeStamp.."\n")
        f:close()
    end
end

function WaitForManualUnlock()
    local validPins = {
        ["21105908"] = "Moises",
        ["21200936"] = "Heriberto",
        ["21201492"] = "Oscar"
    ...
    while not unlocked do
        local enteredPin = PromptForUserInformation(5, "Yield por debajo del 90%", "Llamar al CFT y escanear ID valida para continuar", 8)
        local personName = validPins[enteredPin]
        if personName then
            -- Registro en log
            AppendUnlockLog(enteredPin, personName)

            -- Marcar desbloqueo en archivo
            local file = io.open("C:\\Users\\Public\\Documents\\Cirris\\status\\yield_status.txt", "w")
            if file then
                file:write("OK|"..os.date("%Y-%m-%d %H:%M:%S"))
                file:close()
            end

            -- Mensaje visual
            local msg = DialogOpen(" Acceso concedido. Gracias "..personName)
            Delay(2)
            DialogClose(msg)

            unlocked = true
        else
            local err = DialogOpen(" PIN invalido.")
            Delay(1.5)
            DialogClose(err)
        end
    end
...
function IsYieldLocked()
    local path = "C:\\Users\\Public\\Documents\\Cirris\\status\\yield_status.txt"
    ...
    if file then
        ...
        if content == "LOCKED" then
            return true
        elseif string.sub(content, 1, 2) == "OK" then
            local _, _, savedYield = string.find(content, "OK|[%d%-%s:]+|([%d%.]+)")
            local currentYield = tonumber(GetCurrentYield())
            savedYield = tonumber(savedYield)

            if savedYield and currentYield and currentYield < (savedYield - 5.0) then
                return true -- cayó demasiado desde el desbloqueo manual
            else
                return false
            end
        end
    end
    return false -- por defecto, desbloqueado
end
...
function GetCurrentYield()
    ...
    if file then
        ...
        local _, _, yield = string.find(content, "OK|[%d%-%s:]+|([%d%.]+)")
        return yield or "100"
    end
    return "100"
end
...
function DoOnTestEvent(iEventType)
    if iEventType == 2 and IsYieldLocked() then
        WaitForManualUnlock()
    end
    
    if iEventType == 2 then
        local inputDetected = ReadUserInputStates(3)
        while inputDetected == 1 do -- cambiar a 0 una vez debuggeado el script para no detener al fallar la presión
            local mess = DialogOpen("Advertencia:\n\n\nPresión baja: <40psi.\n\n\nNo puedes continuar probando, favor de llamar a Ingeniería de Pruebas.")
            Delay(1)
            DialogClose(mess)
...
if iEventType == 3 then
    if (bAutoGood == 1) and (GetCableStatus() == 0) then
        ...
        if iCountForClean >= 600 then
            local startTime = os.time() -- Inicio del cronómetro
            local mess = DialogOpen("~Limpieza de impresora~".."\n\nFavor de hacer limpieza de impresora.\n\n\n\n")
            local validPins = { -- Lista de pines correctos con nombres asociados
                    ["4321"] = "Generic Psswd",
                    ["21105908"] = "Moises",
                    ["21200936"] = "Heriberto",
                    ["21201492"] = "Oscar",
            ...
            local isPinValid = false
            local personName = ""

            while not isPinValid do
                local enteredPin = PromptForUserInformation(5, "Favor de hacer limpieza de impresora.", "Escanee ID válida para continuar", 8)
                personName = validPins[enteredPin]
                if personName then
                    isPinValid = true
                else
                    local limpieza = DialogOpen("PIN incorrecto. Intente de nuevo.")
                    Delay(1.5)
                    DialogClose(limpieza)
                end
            end
            local endTime = os.time()
            local elapsedTime = os.difftime(endTime, startTime) -- Tiempo en segundos
            local minutes = math.floor(elapsedTime / 60)
            local seconds = elapsedTime % 60
            local elapsedMessage = "Tiempo: " .. minutes .. " minutos y " .. seconds .. " segundos."
            local correct = DialogOpen("PIN correcto. Gracias " .. personName .. "\n" .. "\n" .. elapsedMessage)
            Delay(5)
            DialogClose(correct)
