...
function PrintStringRAW15charPanther()
    ...
    .."^FT320,690^A0I,60,60^FH\\^FD#06#"..iShift.."D^FS\n"
  
    .."^FT1050,790,^BY5^BCI,260,N,N,N ^FD#07#^FS\n"  -- Ajuste codigo de barras inferior
    .."^FT960,1060^A0I,100,100^FH\\^FD#07#^FS\n"  -- Ajuste posicion ultima linea
    .."^PQ1,0,1,Y^XZ\n"
  ...
  function PrintStringRAW14charPanther()
    local iShift = DetermineShift(os.date("%H:%M"))
    sPrintStringRAW = "CT~~CD,~CC^~CT~\n"
    .."^XA\n"
    .."^PW1224\n"
    ...
    .."^FT1050,690^A0I,60,60^FH\\^FD#05#^FS\n"
    .."^FT320,690^A0I,60,60^FH\\^FD#06#"..iShift.."^FS\n"
  ...
  function PrintStringRAW13charPanther()
    local iShift = DetermineShift(os.date("%H:%M"))
    sPrintStringRAW = "CT~~CD,~CC^~CT~\n"
    .."^XA\n"
    .."^PW1224\n"
...
function PrintStringRAW15char()
    ...
    .."^PW1224\n"
    .."^PON\n"
    .."^FT170,90^A0N,100,100^FH\\^FD#01#^FS\n"                -- Numero de parte de client´de leyenda
...
---------------------------------------------------------------------------------------------------------------------------------------------     
    ...
    elseif sPartNumber == "2154170052" then
        ...
        error("Numero de parte no existe: " .. sPartNumber", favor de contactar a Ing. de Pruebas") 
    end
...
function DoCustomReport()
    ...
    -- Reemplazar datos en la cadena de impresión
...
function DoCustomReport()
    ...
    -- Seleccionar formato de impresión según la longitud del número convertido
    if string.len(numeroEquivalente) == 15 then
        sPrintThis = PrintStringRAW15char()
    elseif string.len(numeroEquivalente) == 14 then
        sPrintThis = PrintStringRAW14char()
    ...
    -- Reemplazar datos en la cadena de impresión y enviar a la impresora
    local tPrintData = DataForPrint()
    sPrintThis = FindAndReplaceInsideString(sPrintThis, tPrintData)
    ...
    if iEventType == 3 then
        if (bAutoGood == 1) and (GetCableStatus() == 0) then
            ...
            if iCountForClean >= 600 then
                local startTime = os.time() -- Inicio del cronometro
                ...
                while not isPinValid do
                    local enteredPin = PromptForUserInformation(5, "Favor de hacer limpieza de impresora.", "Escanee ID valida para continuar", 8)
